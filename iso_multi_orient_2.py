...
class AutoCertISO(Node):
    ...
    def __init__(self):
        ...
        self.num_reps = 10
        self.output_file = 'iso_multi_pose_with_orientation_fixed.csv'
        ...
        self.get_logger().info("🚀 AutoCert ISO (RELATIVE ORIENTATION) Ready")
    ...
    def run(self):
        ...
        for pose_id, (x, y, z) in enumerate(self.poses):
            ...
            samples = []
            ori_samples = []
            ref_q = None   # 🔥 reference orientation
            ...
            for rep in range(self.num_reps):
                ...
                # 🔥 SET REFERENCE ORIENTATION (FIRST VALID)
                if ref_q is None:
                    ref_q = q / np.linalg.norm(q)
                    self.get_logger().info(f"   → Reference orientation set")
